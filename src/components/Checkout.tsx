...
export default function Checkout({
  ...
  const handleValidateAndSubmit = (e: React.FormEvent) => {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      setErrorMessage("Пожалуйста, заполните обязательные поля адреса: Город, Улица и Дом.");
      return;
    }

    if (cart.length === 0) {
      setErrorMessage("Ваша корзина пуста. Добавьте блюда перед оформлением!");
      return;
    }
        ...
        <div className="w-full lg:w-[400px]">
          <div className="bg-surface-container-high rounded-2xl overflow-hidden sticky top-24 shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-outline-variant/20">
            ...
            <div className="p-5 space-y-4 bg-surface-container-high border-t border-outline-variant/15">
              ...
              {errorMessage && (
                <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3 text-red-400 text-[11px] font-bold text-center animate-pulse">
                  {errorMessage}
                </div>
              )}
