      ...
      <div className="max-w-7xl mx-auto px-6 md:px-16 grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
        ...
        <div className="bg-surface-container-high p-8 rounded-2xl shadow-xl border border-outline-variant/20 relative overflow-hidden">
          {subscribed ? (
            ...
            <form onSubmit={handleSubscribe} className="flex flex-col gap-5" id="subscribe-form" noValidate>
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full bg-surface-container-lowest border ${
                      error ? "border-red-400 focus:border-red-400 focus:ring-red-400" : "border-outline-variant focus:border-primary focus:ring-primary"
                    } focus:ring-1 rounded-xl pl-11 pr-4 py-3.5 text-on-surface text-sm outline-none transition-all placeholder:text-on-surface-variant/30 font-medium`}
                ...
                {error && (
                  <p id="newsletter-error" className="text-[10px] text-red-400 font-bold ml-1 animate-fade-in" role="alert">
                    {error}
                  </p>
                )}
              </div>
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-primary text-on-primary py-4 rounded-xl font-display text-xs tracking-widest font-extrabold uppercase hover:glow-orange-sm transition-all active:scale-[0.98] cursor-pointer shadow-lg flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                id="btn-subscribe"
              >
                {isLoading ? (
                  <>
